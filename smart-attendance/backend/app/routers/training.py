...
def _do_training(db: Session):
    ...
    try:
        ...
        # Simulate accuracy calculation based on sample quality
        if total > 0:
            # In a real system, you'd run validation against a test set
...
def start_training(db: Session = Depends(get_db)):
    """Start model training/index rebuilding."""
    if _training_state["is_training"]:
        return {"success": False, "message": "Training already in progress"}
    
    # Run training in background thread
    thread = threading.Thread(target=_do_training, args=(db,), daemon=True)
    thread.start()
...
def rebuild_index(db: Session = Depends(get_db)):
    """Rebuild the face recognition index."""
    try:
        face_service.rebuild_index(db)
...
def get_index_info(db: Session = Depends(get_db), admin = Depends(get_current_admin)):
    ...
    return {
        ...
        "total_embeddings": len(face_service._user_ids),
        "unique_users": len(set(face_service._user_ids)),
        "embedding_dimension": face_service._embeddings.shape[1] if len(face_service._embeddings) > 0 else 0,
        "users_with_embeddings": [
            {"user_id": uid, "name": name, "employee_id": eid}
            for uid, name, eid in zip(
                face_service._user_ids[:20],  # Limit to 20 for response size
